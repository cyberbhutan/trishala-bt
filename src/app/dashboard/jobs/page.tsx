      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Briefcase className="h-6 w-6 text-[#af52de]" />
            <h1 className="text-2xl font-bold text-white">My Job Postings</h1>
          </div>
          <Link href="/jobs/new" className="flex items-center gap-1.5 rounded-xl bg-[#FF8A00] px-4 py-2.5 text-sm font-medium text-white hover:bg-[#E67A00]">
        ...
        {(!items || items.length === 0) ? (
          <div className="rounded-2xl border border-dashed border-white/10 p-16 text-center">
            ...
            <p className="mt-4 text-white/50">No jobs posted yet</p>
            <Link href="/jobs/new" className="mt-3 inline-block text-sm font-medium text-[#FF8A00] hover:text-[#E67A00]">
              Post your first job →
            </Link>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-white/5 bg-white/[0.02]">
                  <th className="px-4 py-3 font-medium text-white/50">Job Title</th>
                ...
                {items?.map((item: any) => (
                  <tr key={item.id} className="border-b border-white/5 transition-colors hover:bg-white/[0.02]">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-xs text-white/40">{item.employment_type} · {item.work_type}</p>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-medium ${
                        ...
                        item.status === 'filled' ? 'bg-blue-500/10 text-blue-400' :
                        'bg-yellow-500/10 text-yellow-400'
                      }`}>{item.status}</span>
                    </td>
                    <td className="px-4 py-3 text-white/50">{item.views_count || 0}</td>
                    <td className="px-4 py-3">
                      <Link href={`/jobs/${item.id}`} className="text-white/30 hover:text-white">
